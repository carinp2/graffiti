...
//        e.preventDefault();
    ...
    $('.my-cart-btn-s-stationary').on('click',function(){
        var itemImg =  $(this).parent().parent().parent().children('.thumb-book-image').find("img").eq(0);
        flyToElement($(itemImg), $('#cart-anchor'));
        addToCartStationary($(this).data("id"));
    });        
...
function addToCartStationary(theId){
   	var insertError = 0;  	        	
   	$.post("order_process.php", {type: "update-cart-stationary", book_id: theId},function(data){
	   	if (data == 0) {
	   		//window.location = vLanguage+"/"+vUrlError;
	   		$('#carteerror').modal('show');
		}
