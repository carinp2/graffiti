$(document).ready(function (){
	...
	//Special sort order start
	$( "#booksLandingSpecialsTable tbody" ).sortable({
	    cancel: ":input,button,[contenteditable]",
	    axis: "y",
	    update: function (event, ui) {
	        var data = $(this).sortable('serialize');
	        $.ajax({
	            data: data,
	            type: 'POST',
	            url: 'book_process.php?type=update-special-rank',
	            success: function(response) {
	            	$("#query-message").html("<h4 class='success-message'><i class='fa fa-check fa-lg space-right green' aria-hidden='true'></i>Sukses! Posisies is verander. Herlaai die blad om die veranderinge te sien.</h4>");
	            },
	            error: function(response) {
  ...
  //Export to Excel end
    ...
    var booksTable = $('#tableBooks').dataTable( {
        ...
        "columns": [{"orderable": false,"searchable": false},null,null,null,{"searchable": false},{"searchable": false},{"searchable": false},{"searchable": false}, {"searchable": false},{"searchable": false},{"searchable": false},
        	{"searchable": false},{"searchable": false},{"searchable": false},null,{"searchable": false},{"searchable": false},{"searchable": false}],
	    fixedHeader: {
	        footer: true
	    }
    });
	...
	var clientsTable = $("#clientsTable").DataTable( {
        "iDisplayLength": 15,
        "aLengthMenu": [[15, 30, 45, -1], [15, 30, 45, "All"]],
        "columns": [null,null,null,null,null,null,{"orderable": false, "searchable": false},null,null,null,null,{"orderable": false, "searchable": false}],
	    fixedHeader: {
	        footer: true
	    }
	});	
	...
	var monthEndTable = $("#monthEndTable").DataTable( {
        "iDisplayLength": 100,
	...
	//Competitions
	var comtetitionsTable = $("#competitionTable").DataTable( {
		"iDisplayLength": 50,
		"aLengthMenu": [[50, 100, 150, -1], [50, 100, 150, "All"]],
		fixedHeader: {
			footer: true
...
function updateSelectString(vId){
	var selectedText = $( "#"+vId+" option:selected" ).text();
	$("#illustrator").val(selectedText);
}
...
$(".saveButton").click( function() {
	...
	var vSoonRank = $("#soon_rank_"+vBookId).val();
	var vNewRank = $("#new_rank_"+vBookId).val();
	var vTv = $("#tv_"+vBookId).val();
	var vTvDate = $("#tv_date_"+vBookId).val();
	var vCostPrice = $("#cost_price_"+vBookId).val();
	var vRr = $("#rr_"+vBookId).val();
	var vRrDate = $("#rr_date_"+vBookId).val();	

	$.post("book_process.php", {type: "edit", book_id: vBookId, price: vPrice, special: vSpecial, special_price: vSpecial_price, bnew: vNew, top_seller: vTop_seller, out_print: vOut_print, in_stock: vInstock, publisher: vPublisher, default_discount:vDefault_discount, date_publish: vPublishDate, soon: vSoon, soon_discount: vSoonDiscount, soon_rank: vSoonRank, new_rank: vNewRank, tv: vTv, tv_date: vTvDate, cost_price: vCostPrice, rr: vRr, rr_date: vRrDate},function(result){
		if(result == 'success'){
...
$("#rotateForm .saveRotateButton").click( function() {
	...
	var vAdvert = $("#advert_"+vRotateId).val();
	var vStartDate = $("#start_date_"+vRotateId).val();
	var vEndDate = $("#end_date_"+vRotateId).val();
	$.post("rotate_process.php", {type: "save-rotate", rotate_id: vRotateId, url: vUrl, alt: vAlt, advert: vAdvert, start_date: vStartDate, end_date: vEndDate},function(result){
    	if(result == 'success'){
...
$("#rotateForm #addRotateButton").click( function() {
	...
	var vAdvert = $("#rotateForm #new_advert").val();
	var vNewStartDate = $("#rotateForm #new_start_date").val();
	var vNewEndDate = $("#rotateForm #new_end_date").val();
    ...
    if(vAdvert == 1){
    	if(vNewStartDate.length == 0){
	        var error = true;
	        $('#rotateForm #new_start_date').addClass("validation");
    	}
    	else {
    		$('#rotateForm #new_start_date').removeClass("validation");
    	}
    	if(vNewEndDate.length == 0){
	        var error = true;
	        $('#rotateForm #new_end_date').addClass("validation");
    	}
    	else {
    		$('#rotateForm #new_end_date').removeClass("validation");
    	}    	
    } else if(vAdvert == 0) {//Not advert
    	$('#rotateForm #new_end_date').removeClass("validation");
    	$('#rotateForm #new_start_date').removeClass("validation");
    }    
...
$("[id^=courier_selected_]").on('change', function() {
	var vOrderId = $(this).attr('data-id');
	// alert(vOrderId);
	// alert($(this).children("option:selected").val());
	if($(this).children("option:selected").val() == "internet_express"){
		$("#createWaybill_"+vOrderId).removeClass('no-display');
	}
	else{
		$("#createWaybill_"+vOrderId).addClass('no-display');
	}
});

//Generate Internet Express waybill
$("[id^=createWaybill_]").click( function() {
	var vOrderId = $(this).attr('data-id');
	$.post("waybill_internet_exp.php", {type: "posted", order_id: vOrderId, },function(result){
		if (result == "success") {
			alert(result);
		 }
		else{
			alert("Waybill nie geskep nie - probeer weer.");
		}
	});
});
...
$("[id^=tracking_email_]").on('click', function() {
	...
	var vCourierSelected = $("#courier_selected_"+vOrderId).val();
	if(vPosted == 51 || vPosted == 53){
		var vClientId = $(this).attr('data-client');
		...
		if(vTracking.length == 0){
			...
			$("#pre_load").fadeIn("slow");
				   $.post("cms_order_process.php", {type: "posted", order_id: vOrderId, client_id: vClientId, courier_type: vCourierType, salt: vOrderSalt, tracking: vTracking, posted: vPosted, courier_selected: vCourierSelected},function(result){
					   	if (result == "success") {
					   		location.reload();
					     }
					   	else{
...
$("[id^=printInvoiceButton2_]").on('click', function() {
	var vOrderId = $(this).attr('data-id');
	var vClientId = $(this).attr('data-clientid');
	$vUrl = "cms_order_process.php?type=invoice_print_wr&order_id="+vOrderId+"&client_id="+vClientId;
	window.open($vUrl,"printWindow");
	   //$.post("cms_order_process.php", {type: "invoice_print", order_id: vOrderId, client_id: vClientId},function(result){
		//  });       	
});
...
$("[id^=saveOrder_]").on('click', function() {
	...
	var vPosted = $("#posted_"+vOrderId).val();
	   $.post("cms_order_process.php", {type: "order", order_id: vOrderId, note: vNote, courier_type: vCourierType, courier_cost: vCourierCost, total_cost: vTotalCost, tracking_no: vTrackingNo, payment_type: vPaymentType, posted: vPosted},function(result){
	    	if(result == 'success'){
...
$("[id^=saveClient_]").on('click', function() {
	...
	var vValidated = $("#validated_"+vClientId).val();
	
   $.post("clients_process.php", {type: "edit", client_id: vClientId, special_discount: vSpecial_discount, validated: vValidated},function(result){
    	if(result == 'success'){
...
function checkDocType(vId, vValue, vType, vForm) {
	var ext = vValue.match(/\.(.+)$/)[1].toLowerCase();
	if(vType == 'img') {
		switch (ext) {
			case 'jpg':
			case 'jpeg':
			case 'png':
			case 'gif':
				$('#'+vForm+' #saveButton'+vId).fadeIn(500);
				$('#fileError'+vId).fadeOut(500);
				$('#blob_path'+vId).removeClass("validation");

				if (window.File && window.FileReader && window.FileList && window.Blob) {
					var fsize = $('#blob_path'+vId)[0].files[0].size;
					if (fsize <= 1048576) {
						$('#fileSizeError'+vId).fadeOut(500);
						$('#blob_path'+vId).removeClass("validation");
						$('#'+vForm+' #saveButton'+vId).fadeIn(500);
					} else {
						$('#fileError'+vId).fadeOut(500);
						$('#blob_path'+vId).addClass("validation");
						$('#fileSizeError'+vId).fadeIn(500);
						$('#'+vForm+' #saveButton'+vId).fadeOut(500);
					}
				} else {
					alert("Please upgrade your browser, because your current browser lacks some new features we need!");
				}
	...
	$("#submitBook").click( function() {
        ...
        if(author.length == 0){
            var error = true;
            $('#author').addClass("validation");
        } else {        	
            $('#author').removeClass("validation");
        }
        if(price.length == 0){
            var error = true;
	...
	$("#searchBook").click( function() {
    	...
    	$('#searchStationaryForm').fadeOut(500);
	});
	...
	//Search Stationary per Supplier start
	$("#searchStationary").click( function() {
		$('#searchStationaryForm').fadeIn(500);  
    	$('#subCategoryBookForm').fadeOut(500);  
    	$('#publisherBookForm').fadeOut(500);  
    	$('#searchBookForm').fadeOut(500);  
    	$('#authorBookForm').fadeOut(500);  
	...
	$("#searchStationaryForm #supplier_id").change(function () {
		var vSupplier = $("#searchStationaryForm #supplier_id").val();
	    if(vSupplier > 0){
	    	document.location.href = "index.php?page=books&type=searchStationarySupplier&id="+vSupplier;
	    }
	});    
	//Search Stationary per Supplier end		
	...
	//Search Clients start
	$("#clientSearch1").click( function() {
		$('#clientSearchDiv').fadeIn(500);  
	});
	
	$("#clientForm #clientSubmit2").click(function () {
    	event.preventDefault();
		var vSearch = $("#clientForm2 #client").val();
	    if(vSearch.length > 0){
	    	$("#clientForm2").submit();
	    }
	});    
	...
	$('#loadImagesForm [id^=images]').change(function () {
		var fileTypeError = false;
		var fileSizeError = false;
		var filePhysicalSizeError = false;
		var fileNumberError = false;
		var files = event.target.files;
		for (var i=0; i < files.length; i++) {
			  	var fileName =  files[i].webkitRelativePath;
			   	var ext = fileName.match(/\.(.+)$/)[1].toLowerCase();
		...
		if(fileTypeError == true){
			loadImagesForm
        	$("#loadImagesForm #submitFile").fadeOut(500);  
		}
		else {
        	$("#loadImagesForm #fileImagesError").fadeOut(500);  
        	$("#loadImagesForm #submitFile").fadeIn(500);          	
		}
		if(fileSizeError == true){
        	$("#loadImagesForm #fileImagesSizeError").fadeIn(500);  
        	$("#loadImagesForm #submitFile").fadeOut(500);          	
		}
		else {
        	$("#loadImagesForm #fileImagesSizeError").fadeOut(500);  
        	$("#loadImagesForm #submitFile").fadeIn(500);          	
		}		
		if(fileNumberError == true){
        	$("#loadImagesForm #fileImagesNumberError").fadeIn(500);  
        	$("#loadImagesForm #submitFile").fadeOut(500);          	
		}
		else {
        	$("#loadImagesForm #fileImagesNumberError").fadeOut(500);  
        	$("#loadImagesForm #submitFile").fadeIn(500);          	
		}				
	});		
	//Batch  functions end	
...
function updateCompetition(vId) {
	// var vId = $(this).data("id");
	var vCompName = $('#name' + vId).val();
	var vCompDescription = $('#description' + vId).val();
	var vCompDateEnd = $('#date_end' + vId).val();
	var vCompValid = $('#valid' + vId).val();
	var vWinner = $('#winner' + vId).val();
	...
	$.ajax({
		data: {
			...
			id: vId,
			name: vCompName,
			description: vCompDescription,
			date_end: vCompDateEnd,
			winner: vWinner,
			valid: vCompValid
		},
		type: 'POST',
		url: 'competition_process.php',
		success: function (response) {
			$("#com_tr_"+ vId).children('td').removeClass("bg-success");
			$("#com_tr_"+ vId).children('td').removeClass("bg-error");
			if(response == 1) {
				$("#com_tr_"+ vId).children('td').addClass("bg-success");
				$("#query-message").html("<h4 class='success-message'><i class='fa fa-check fa-lg space-right green' aria-hidden='true'></i>Sukses! Kompetisie is verander.</h4>");
			}
			else if(response == 0){
				$("#com_tr_"+ vId).children('td').addClass("bg-error");
				$("#query-message").html("<h4 class='error-message'><i class='fa fa-times fa-lg space-right red' aria-hidden='true'></i>Fout! Kompetisie is nie verander nie.</h4>");
			}
		},
		error: function (response) {
			$("#com_tr_"+ vId).children('td').addClass("bg-error");
...
$('[id^=winnerSelect]').on('change',function() {
	var vId = $(this).attr('data-id');
	var vValue = $(this).val();
	var vText = "<u>Selected</u>:<br>- "+$(this).children("option:selected").map(function () {
		return $(this).text();
    }).get().join('<br>- ');
	$('#winner_display'+vId).html(vText);
	$('#winner'+vId).val(vValue);
});
